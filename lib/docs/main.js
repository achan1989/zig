(function() {
    ...
    function enableLogging(truefalse) {
      loggingEnabled = truefalse;
    }
    ...
    function render() {
        ...
        if (curNavSearch !== "") {
          console.log("Render::renderSearch()");
          return renderSearch()
        };

        switch (curNav.tag) {
          case NAV_home:
            console.log("Render::renderHome()");
            return renderHome();
          case NAV_decl:
            if (curNav.decl == null) {
              console.log("Render::renderNotFound()");
              return renderNotFound();
            } else {
              console.log("Render::renderDecl()", curNav.decl);
              return renderDecl(curNav.decl);
            }
          case NAV_source:
            console.log("Render::renderSource()");
    ...
    // Render the first header below the nav bar:
    // "struct [src]", "Type [src]", etc.
    ...
    // Render the breadcrumb bar under the search.
      ...
      member_loop: for (let i = 0; i < members.length; i += 1) {
        ...
        while (true) {
          ...
          switch (member_category) {
            ...
            case CAT_type_type:
            case CAT_type_function:
              typesList.push({original: original, member: member, nameOriginal: declIndexName(original), nameMember: declIndexName(member)});
              continue member_loop;
            case CAT_error_set:
    ...
    function renderNamespacePage(decl_index) {
      console.log("renderNamespacePage()", decl_index);
      renderNav(decl_index);
      renderDeclHeading(decl_index);
    ...
    function updateCurNav(location_hash) {
        ...
        if (location_hash.length > 1 && location_hash[0] === '#') {
            ...
            if (nonSearchPart.length > 0) {
              ...
              if (source_mode) {
                curNav.tag = NAV_source;
                curNav.path = nonSearchPart.substring(4);
              } else {
                console.log("Nav change, findDecl() for", nonSearchPart);
                curNav.tag = NAV_decl;
    ...
    function doDeclTest() {
      const fqn479 = fullyQualifiedName(479);
      const path479 = unwrapString(wasm_exports.decl_file_path(479));
      const fqn225 = fullyQualifiedName(225);
      const path225 = unwrapString(wasm_exports.decl_file_path(225));
      console.log({
        fqn479: fqn479,
        path479: path479,
        fqn225: fqn225,
      ...
      const numDecls = wasm_exports.num_decls();
      console.log("%i decls", numDecls);
      var numWithSameResult = 0;
      var numWithDifference = 0;
      for (let i = 0; i < numDecls; i += 1) {
        if (i == 1000) return;
        if (!wasm_exports.decl_is_public(i)) continue;

        const originalDecl = i;
        var resolvedDecl = originalDecl;

        // Resolve any decl alias in the same way that renderNamespace() does.
        inner: while (true) {
          const declCategory = wasm_exports.categorize_decl(resolvedDecl, 0);
          switch (declCategory) {
            case CAT_alias:
              resolvedDecl = wasm_exports.get_aliasee();
              continue;
            case CAT_namespace:
            case CAT_container:
            case CAT_global_variable:
            ...
            case CAT_primitive:
              break inner;
            default:
              throw new Error("uknown category: " + declCategory);
          }
        }

        const originalFqn = fullyQualifiedName(originalDecl);
        const resolvedFqn = fullyQualifiedName(resolvedDecl);
        const findOriginal = findDecl(originalFqn);
        const findResolved = findDecl(resolvedFqn);
        //if ((originalDecl != findOriginal) || (resolvedDecl != findResolved)) {
        if (findOriginal != findResolved) {
          numWithDifference += 1;
          console.log("Difference", {
            originalDecl: originalDecl,
            originalName: declIndexName(originalDecl),
            originalFqn: originalFqn,
            findOriginal: findOriginal,
            resolvedDecl: resolvedDecl,
            resolvedName: declIndexName(resolvedDecl),
      ...
      const numPublicDecls = numWithSameResult + numWithDifference;
      console.log("%i public decls, %i with same result, %i different",
        numPublicDecls, numWithSameResult, numWithDifference);
    }
})();
